...
(function() {
  var MOON_DATA, Moon, PI, TIME_CONSTANT, TIME_OFFSET, TWO_PI, animate, coord_to_pixel, moons, parse_moons;
  ...
  Moon = class Moon {
    ...
    position_at_time(t) {
      ...
      var E, M, n, r, result, theta, x, y;
      t += TIME_OFFSET; // Compensate for start time
      n = TWO_PI / this.period; // mean motion
      M = n * t; // mean anomaly
      ({
        kepler_root_fn: function(E_) {
          return (E_ - this.ecc * np.sin(E_) - M) ** 2;
        }
      });
  ...
  animate = function() {
    var TIMESCALE, s, svgDoc;
    TIMESCALE = 1e3;
    // Draw moons
    svgDoc = d3.select("body").append("svg");
    s = svgDoc.append("g").selectAll("circle").data(moons);
    s.exit().remove();
    return s.enter().append("circle").attr("r", function(m) {
      return 2;
    }).transition().duration(10000).attrTween("cx", function(m) {
      return function(t) {
        return coord_to_pixel(m.position_at_time(t * TIMESCALE)[0]);
      };
    }).attrTween("cy", function(m) {
      return function(t) {
        return coord_to_pixel(m.position_at_time(t * TIMESCALE)[1]);
      };
    });
  };
